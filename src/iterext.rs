...
pub trait IterExt : Iterator {
    ...
    fn scan_ref<St, F>(self, init: St, f: F) -> Scan<Self, (F, St), fn(&mut (F, St), Self::Item) -> Option<St>>
        where Self: Sized, St: Copy,F: FnMut(&mut St, Self::Item),
    {
        fn scan_ref_closure<St, F, I>(f_st: &mut (F, St), item: I) -> Option<St>
            where St: Copy, F: FnMut(&mut St, I)
        {
    ...
    fn fold_ref<B, F>(self, init: B, mut f: F) -> B
        where Self: Sized, F: FnMut(&mut B, Self::Item)
    {
    ...
    fn fold_to_map<K, V, F>(self, mut f: F) -> HashMap<K, V>
        where Self: Sized, K: Eq + Hash, F: FnMut(&mut HashMap<K, V>, Self::Item)
    {
